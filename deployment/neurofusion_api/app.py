...
# Feature Extraction — fully vectorised, ~2–3 ms per window
...
def extract_features(window: np.ndarray) -> np.ndarray:
    ...
    # Spectral features
    freqs = np.fft.rfftfreq(N, d=1.0 / 256)
    ...
    # Permutation entropy — vectorised (was O(N) Python loop, now pure numpy)
    order = 3
    Np    = N - order + 1
    idx   = np.arange(order) + np.arange(Np)[:, None]
    patterns = np.argsort(x[idx], axis=1)
    base     = np.array([order ** i for i in range(order)])
    keys     = (patterns * base).sum(axis=1)
    _, counts = np.unique(keys, return_counts=True)
    probs     = counts / counts.sum()
    perm_ent  = float(-np.sum(probs * np.log2(probs + eps)))
 
    # Sample entropy — fast O(N) approximation
    samp_ent = float(np.std(dx) / (std_dev + eps))
 
    # Hurst exponent
    lags     = range(2, min(20, N // 2))
    tau      = [np.std(x[lag:] - x[:-lag]) for lag in lags]
    poly     = np.polyfit(np.log(lags), np.log(np.array(tau) + eps), 1)
    hurst_exp = float(poly[0])
 
    # Higuchi fractal dimension — inner loop vectorised
    kmax = 10
    L    = []
    for k in range(1, kmax + 1):
        Lk = []
        for m in range(1, k + 1):
            idxh = np.arange(m - 1, N - k, k)
            if len(idxh) < 2:
                continue
            s   = np.sum(np.abs(x[idxh[1:]] - x[idxh[:-1]]))
            n_m = len(idxh) - 1
            Lk.append(s * (N - 1) / (k * n_m * k))
        if Lk:
            L.append(np.mean(Lk))
    poly2   = np.polyfit(np.log(range(1, len(L) + 1)), np.log(np.array(L) + eps), 1)
    higuchi = float(-poly2[0])
 
    # Katz fractal dimension
    L_total = np.sum(np.abs(dx))
...
scaler.mean_ = np.array([
    ...
    48.061375326554156, 0.010061946344309554, 0.05650263504499272, 0.052505202381546386,
    0.17127479002365775, 0.21815379790124503, 1.1862835197639954, 0.058971321441228734,
    0.3336417535813464, 219.37455139431884, 4.097705864540616, 218.58484430155596,
    1.4444528982904365, 1.2023933023964997, 5.532397359867166, 14.686729211501522,
    218.58484430155596, 4.528676511747044, 22.69786712691089, 63845.01512878856,
    15.696826452013946, 110.63214884106557, 43.2212418252191, 1.042137506191448,
    0.36519616225320334, 2.0871109719669176
])
scaler.scale_ = np.array([
    1.289966526495437, 3.7023801120755357, 4.568841799492872, 13.464123049699042,
    16.407462234276327, 0.005127759772553031, 0.009467616944799079, 0.015381402086271433,
    0.015379090533348945, 0.014419922376115727, 0.44700999742033687, 0.030057533215603723,
    0.07039453761296219, 70.54860141895661, 0.12329599672216021, 50.4883468624754,
    0.010617287547115106, 0.006618659205429603, 0.4498060668852064, 1.6984785455174038,
    50.4883468624754, 0.4717109572128019, 6.278218014091133, 14082.989925883561,
    1.7332713815754344, 20.566589949563827, 2.851996736609765, 0.08092920970969399,
    0.04069000729557236, 0.10329730242016892
])
...
@app.get("/")
def root():
    return {"name": "NeuroFusion-EEG API", "version": "1.0.0", "status": "running"}
...
def info():
    return {
        ...
        "n_features"   : 30,
        "classes"      : CLASS_NAMES,
        "performance"  : {"macro_f1": 0.2763, "accuracy": 0.2824, "auc": 0.5016},
        "disclaimer"   : "Research purposes only. Not a medical device.",
    }
...
def predict_single_window(request: SignalRequest):
    ...
    try:
        result = run_prediction(np.array(request.signal, dtype=np.float64))
...
async def predict_from_csv(file: UploadFile = File(...)):
    ...
    try:
        ...
        if len(windows) == 0:
            raise HTTPException(
                status_code=422,
                detail=f"Signal must be at least {WINDOW_SIZE} samples.",
            )
        ...
        return {
            ...
            "dominant_prediction": {"class": dominant, "label": CLASS_NAMES[dominant]},
            "disclaimer"         : "Research purposes only. Not a medical device.",
            "windows"            : results,
        }
...
def predict_batch_windows(signals: List[SignalRequest]):
    ...
    try:
        ...
        for i, req in enumerate(signals):
            ...
            r = run_prediction(np.array(req.signal, dtype=np.float64))
            r["window_index"] = i
            results.append(r)
            class_counts[r["predicted_class"]] += 1
